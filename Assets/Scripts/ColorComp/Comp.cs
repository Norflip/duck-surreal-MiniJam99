...
*/
    ...
    {
        RenderTexture a = RoundTextureToPalette(rt, image.palette);
        RenderTexture b = RoundTextureToPalette(image.image, image.palette);
        int maxValue = image.image.width * image.image.height * 10;
        int[] c= new int[1]{0};
        ComputeBuffer counter = new ComputeBuffer(1, sizeof(int));
        counter.SetData(c);

        compare_cs.SetTexture(0, "_TextureA", a);
        compare_cs.SetTexture(0, "_TextureB", b);
        compare_cs.SetBuffer(0, "_Counter", counter);
        ...
        float percentage = (float)c[0] / (image.image.width * image.image.height * 3); //1.0f - (c[0] / 255.0f) / (m0.image.width * m0.image.height);
        Debug.Log("percentage: " + percentage);
        Debug.Log("c: " + c[0]);
    ...
    public RenderTexture RoundTextureToPalette (Texture image, Color[] palette)
    {
        RenderTexture rt = new RenderTexture(image.width, image.height, 0);
        rt.enableRandomWrite = true;
        rt.filterMode = FilterMode.Point;
        rt.Create();

        ComputeBuffer paletteBuffer = new ComputeBuffer(palette.Length, sizeof(float) * 4);
        paletteBuffer.SetData(palette);

        round_cs.SetTexture(0, "_Source", image);
        round_cs.SetTexture(0, "_Result", rt);
        round_cs.SetBuffer(0, "_Palette", paletteBuffer);

        int thrx = Mathf.CeilToInt(image.width / 8.0f);
        int thry = Mathf.CeilToInt(image.height / 8.0f);
